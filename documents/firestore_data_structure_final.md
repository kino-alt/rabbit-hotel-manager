# Firestoreデータ構造の設計メモ(改訂履歴・判断の記録)
...
- **`photoSchedule`(日ごとの写真要否を予定として持つ)**:書き込む画面が実装されず常に空だったため改訂4で廃止。自動判定に一本化
- **`careTotals`/`runTotal`(宿泊全体の累計フィールド)**:どの画面も参照せず、当日記録の書き込みと非アトミックにズレる原因だったため改訂4で廃止
